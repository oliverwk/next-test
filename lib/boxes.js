...
export let boxMakeToken = async () => {
    let key = {
      key: (process.env.dev === "true") ? Buffer.from(process.env.privateKey, 'base64') : process.env.privateKey,
    ...
    // We are using the excellent builtin function fetch
...
export let boxGetFolder = async (id = 132260108317, offset = 0) => {
  ...
  if (!globalThis.accessToken) {
    ...
    console.log("AccessToken:", globalThis.accessToken);
    let data = await fetch(`https://api.box.com/2.0/folders/${id}/items/?fields=id%2Cname%2Cdownload_url&offset${Math.round(offset)}&limit=60`, {
      headers: {
        'as-user': process.env.usid,
        'Authorization': `Bearer ${globalThis.accessToken}`
      }
    ...
    console.log("Id:", id); //ID: 114207700943
    console.log("offset:", Math.round(offset));
    console.log("AccessToken:", globalThis.accessToken);
    let data = await fetch(`https://api.box.com/2.0/folders/${id}/items/?fields=id%2Cname%2Cdownload_url&offset${Math.round(offset)}&limit=60`, {
      headers: {
        'as-user': process.env.usid,
